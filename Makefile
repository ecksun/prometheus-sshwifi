...
$(PACKAGE_DIR): \
	...
	$(PACKAGE_DIR)/usr/bin/prometeus-sshwifi/process.sh \
	$(PACKAGE_DIR)/usr/bin/prometeus-sshwifi/scan.sh \
	$(PACKAGE_DIR)/usr/bin/prometeus-sshwifi/scan_and_process.sh \
...
$(PACKAGE_DIR)/usr/bin/prometeus-sshwifi/%.sh: %.sh
	@mkdir -p "$(dir $@)"
	cp -p "$<" "$@"
